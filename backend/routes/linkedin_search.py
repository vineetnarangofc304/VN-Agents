...
async def generate_message_script(data: dict):
    ...
    script = (
        "// === LinkedIn Bulk Messenger v5 ===\n"
        "// Uses fs_miniProfile URN format for messaging (not fsd_profile)\n"
        ...
        "  const HP = {...H, 'content-type': 'application/json', 'accept': 'application/vnd.linkedin.normalized+json+2.1'};\n\n"
        "  console.log('%c=== Bulk Messenger v5: ' + recipients.length + ' recipients ===%c', 'color:#0a66c2;font-size:14px;font-weight:bold', '');\n\n"
        "  // Helper: convert fsd_profile URN to fs_miniProfile URN\n"
        "  const toMiniUrn = (urn) => {\n"
        "    if (urn.includes('fs_miniProfile')) return urn;\n"
        "    const id = urn.split(':').pop();\n"
        "    return 'urn:li:fs_miniProfile:' + id;\n"
        "  };\n\n"
        "  for (let i = 0; i < recipients.length; i++) {\n"
        "    const rec = recipients[i];\n"
        "    console.log('(' + (i+1) + '/' + recipients.length + ') ' + rec.name);\n\n"
        "    let rawUrn = rec.entity_urn || '';\n\n"
        "    // URN lookup via dash endpoint\n"
        ...
        "              rawUrn = item.entityUrn; break;\n"
        "            }\n"
        "          }\n"
        "          if (!rawUrn) {\n"
        "            const elems = dd.data?.elements || dd.elements || [];\n"
        "            for (const el of elems) {\n"
        "              if (el.entityUrn?.includes('profile')) { rawUrn = el.entityUrn; break; }\n"
        "            }\n"
        "          }\n"
        "        } else { console.log('  Dash lookup: HTTP ' + dResp.status); }\n"
        "      } catch(e) { console.log('  Lookup error: ' + e.message); }\n"
        "    }\n\n"
        "    if (!rawUrn) { skipped++; console.log('  SKIP — no URN'); await delay(500); continue; }\n\n"
        "    // Extract member ID and try both URN formats\n"
        "    const memberId = rawUrn.split(':').pop();\n"
        "    const miniUrn = 'urn:li:fs_miniProfile:' + memberId;\n"
        "    const fsdUrn = 'urn:li:fsd_profile:' + memberId;\n"
        "    console.log('  Member: ' + memberId);\n\n"
        "    // Send message — try multiple URN formats and methods\n"
        "    try {\n"
        "      let ok = false;\n\n"
        "      // Attempt 1: fs_miniProfile URN with keyVersion\n"
        "      let resp = await fetch('https://www.linkedin.com/voyager/api/messaging/conversations?action=create', {\n"
        ...
        "        })\n"
        "      });\n"
        "      if (resp.ok || resp.status === 201) { ok = true; console.log('  Method 1 OK (miniProfile)'); }\n"
        "      else { console.log('  M1(' + resp.status + ') miniProfile'); }\n\n"
        "      // Attempt 2: fsd_profile URN with keyVersion\n"
        "      if (!ok) {\n"
        ...
        "          })\n"
        "        });\n"
        "        if (resp.ok || resp.status === 201) { ok = true; console.log('  Method 2 OK (fsdProfile)'); }\n"
        "        else { console.log('  M2(' + resp.status + ') fsdProfile'); }\n"
        "      }\n\n"
        "      // Attempt 3: member URN format\n"
        "      if (!ok) {\n"
        ...
        "          })\n"
        "        });\n"
        "        if (resp.ok || resp.status === 201) { ok = true; console.log('  Method 3 OK (member)'); }\n"
        "        else {\n"
        "          const et = await resp.text().catch(() => '');\n"
        "          console.log('  M3(' + resp.status + '): ' + et.substring(0, 100));\n"
        "        }\n"
        "      }\n\n"
        "      if (ok) {\n"
        "        sent++;\n"
        ...
        "      } else {\n"
        "        failed++;\n"
        "        console.log('%c  ALL METHODS FAILED%c', 'color:#ef4444;font-weight:bold', '');\n"
        "      }\n"
