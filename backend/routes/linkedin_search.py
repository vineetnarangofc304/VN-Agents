...
async def generate_message_script(data: dict):
    ...
    script = (
        "// === LinkedIn Bulk Messenger v4 ===\n"
        "// Fixed payload format with keyVersion + body + attributedBody\n"
        ...
        "  console.log('%c=== Bulk Messenger v4: ' + recipients.length + ' recipients ===%c', 'color:#0a66c2;font-size:14px;font-weight:bold', '');\n\n"
        "  for (let i = 0; i < recipients.length; i++) {\n"
        "    const rec = recipients[i];\n"
        "    console.log('(' + (i+1) + '/' + recipients.length + ') ' + rec.name);\n\n"
        "    let urn = rec.entity_urn || '';\n\n"
        "    // URN lookup: try dash endpoint first (profileView is 410 Gone)\n"
        ...
        "            }\n"
        "          }\n"
        "          if (urn) console.log('  URN: ' + urn);\n"
        "        } else { console.log('  Dash lookup: HTTP ' + dResp.status); }\n"
        "      } catch(e) { console.log('  Lookup error: ' + e.message); }\n"
        "    }\n\n"
        "    if (!urn) {\n"
        "      skipped++;\n"
        "      console.log('  SKIP — no URN found');\n"
        "      await delay(500);\n"
        "      continue;\n"
        "    }\n\n"
        "    // Send message — correct LinkedIn payload format\n"
        "    try {\n"
        "      let ok = false;\n\n"
        "      // Method: conversations?action=create with keyVersion + body + attributedBody\n"
        ...
        "        console.log('  Method 1 (' + resp.status + '): ' + errText.substring(0, 120));\n"
        "      }\n\n"
        "      // Fallback: try without keyVersion\n"
        "      if (!ok) {\n"
        "        const resp2 = await fetch('https://www.linkedin.com/voyager/api/messaging/conversations?action=create', {\n"
        "          method: 'POST', headers: HP,\n"
        "          body: JSON.stringify({\n"
        "            conversationCreate: {\n"
        "              eventCreate: {\n"
        ...
        "          })\n"
        "        });\n"
        "        if (resp2.ok || resp2.status === 201) { ok = true; }\n"
        "        else {\n"
        "          const et = await resp2.text().catch(() => '');\n"
        "          console.log('  Method 2 (' + resp2.status + '): ' + et.substring(0, 120));\n"
        "        }\n"
        "      }\n\n"
        "      // Fallback 2: try newer dash endpoint\n"
        "      if (!ok) {\n"
        "        const resp3 = await fetch('https://www.linkedin.com/voyager/api/voyagerMessagingDashMessengerMessages?action=createMessage', {\n"
        "          method: 'POST', headers: HP,\n"
        "          body: JSON.stringify({\n"
        "            message: {\n"
        ...
        "              originToken: ''\n"
        "            },\n"
        "            mailboxUrn: 'urn:li:fsd_profile:' + csrf.split('ajax:')[1]?.split('\"')[0],\n"
        "            hostRecipientUrns: [urn],\n"
        "            dedupeByClientGeneratedToken: false\n"
        "          })\n"
        "        });\n"
        "        if (resp3.ok || resp3.status === 201) { ok = true; }\n"
        "        else {\n"
        "          const et = await resp3.text().catch(() => '');\n"
        "          console.log('  Method 3 (' + resp3.status + '): ' + et.substring(0, 120));\n"
        "        }\n"
        "      }\n\n"
        "      if (ok) {\n"
        "        sent++;\n"
        ...
        "      } else {\n"
        "        failed++;\n"
        "        console.log('%c  FAILED%c', 'color:#ef4444;font-weight:bold', '');\n"
        "      }\n"
