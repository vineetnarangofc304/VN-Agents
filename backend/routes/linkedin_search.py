...
async def generate_message_script(data: dict):
    """Generate a browser console script that sends messages using LinkedIn's messaging UI overlay."""
    ...
    script = (
        ...
        "// Uses LinkedIn's messaging overlay — no page navigation\n\n"
        "(async () => {\n"
        ...
        "    const name = r.name;\n"
        "    console.log('(' + (i+1) + '/' + recipients.length + ') ' + name);\n"
        "    try {\n"
        "      // Step 1: Open new message compose\n"
        "      const composeBtn = document.querySelector('a[href=\"/messaging/thread/new/\"]')\n"
        "                      || document.querySelector('.msg-overlay-bubble-header__control--new-convo-btn')\n"
        "                      || document.querySelector('[data-control-name=\"overlay.compose_message\"]');\n"
        "      if (composeBtn) { composeBtn.click(); await delay(1500); }\n"
        "      else {\n"
        "        // Click messaging icon first\n"
        "        const msgIcon = document.querySelector('a[href*=\"/messaging\"]')\n"
        "                     || document.querySelector('#messaging-tab-icon');\n"
        "        if (msgIcon) { msgIcon.click(); await delay(1500); }\n"
        "        const newBtn = document.querySelector('a[href=\"/messaging/thread/new/\"]')\n"
        "                    || document.querySelector('.msg-overlay-bubble-header__control--new-convo-btn');\n"
        "        if (newBtn) { newBtn.click(); await delay(1500); }\n"
        "      }\n\n"
        "      // Step 2: Type recipient name in the To field\n"
        "      const toInput = document.querySelector('input[name=\"audience\"]')\n"
        "                   || document.querySelector('.msg-connections-typeahead__search-field')\n"
        "                   || document.querySelector('input[placeholder*=\"name\"]')\n"
        "                   || document.querySelector('input.msg-compose-form__recipients-input');\n"
        "      if (!toInput) { failed++; console.log('  To field not found'); continue; }\n"
        "      toInput.focus();\n"
        "      toInput.value = '';\n"
        "      toInput.dispatchEvent(new Event('input', {bubbles: true}));\n"
        "      await delay(500);\n"
        "      // Type name character by character for typeahead\n"
        "      const typeName = name.split(' ')[0] + ' ' + (name.split(' ')[1] || '');\n"
        "      for (const ch of typeName.trim()) {\n"
        "        toInput.value += ch;\n"
        "        toInput.dispatchEvent(new Event('input', {bubbles: true}));\n"
        "        await delay(50);\n"
        "      }\n"
        ...
        "      const suggestion = document.querySelector('.msg-connections-typeahead__suggestion')\n"
        "                      || document.querySelector('.basic-typeahead__triggered-content li')\n"
        "                      || document.querySelector('[role=\"listbox\"] [role=\"option\"]');\n"
        "      if (!suggestion) { failed++; console.log('  No suggestion found for ' + name); \n"
        "        // Close compose\n"
        "        const closeBtn = document.querySelector('.msg-overlay-bubble-header__control--close-btn, button[data-control-name=\"overlay.close_convo_window\"]');\n"
        "        if (closeBtn) closeBtn.click();\n"
        "        await delay(1000); continue; }\n"
        "      suggestion.click();\n"
        "      await delay(1500);\n\n"
        "      // Step 4: Type message\n"
        "      const msgBox = document.querySelector('div.msg-form__contenteditable[contenteditable=\"true\"]')\n"
        "                  || document.querySelector('.msg-form__msg-content-container div[contenteditable]')\n"
        "                  || document.querySelector('div[role=\"textbox\"]');\n"
        "      if (!msgBox) { failed++; console.log('  Message box not found'); continue; }\n"
        "      msgBox.focus();\n"
        "      msgBox.innerHTML = '<p>' + message.replace(/\\n/g, '</p><p>') + '</p>';\n"
        "      msgBox.dispatchEvent(new Event('input', {bubbles: true}));\n"
        "      await delay(1000);\n\n"
        "      // Step 5: Click send\n"
        "      const sendBtn = document.querySelector('button.msg-form__send-button:not([disabled])')\n"
        "                   || document.querySelector('button.msg-form__send-btn:not([disabled])');\n"
        "      if (sendBtn) {\n"
        "        sendBtn.click(); sent++;\n"
        "        console.log('  Sent to ' + name);\n"
        "      } else {\n"
        "        // Try any visible Send button\n"
        "        let clicked = false;\n"
        "        document.querySelectorAll('button').forEach(b => {\n"
        "          if (!clicked && b.innerText.trim() === 'Send' && !b.disabled) { b.click(); clicked = true; sent++; }\n"
        "        });\n"
        "        if (!clicked) { failed++; console.log('  Send button not found/disabled'); }\n"
        "      }\n"
        "      await delay(1500);\n\n"
        "      // Step 6: Close the compose window\n"
        "      const closeBtn = document.querySelector('.msg-overlay-bubble-header__control--close-btn')\n"
        "                    || document.querySelector('button[data-control-name=\"overlay.close_convo_window\"]');\n"
        "      if (closeBtn) closeBtn.click();\n"
