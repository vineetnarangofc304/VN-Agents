...
async def generate_message_script(data: dict):
    """Generate a browser console script that sends messages via LinkedIn's internal API."""
    ...
    # This script uses LinkedIn's Voyager API directly from the browser
    # Since user is logged into LinkedIn, all cookies are sent automatically
    script = (
        ...
        "    console.log('(' + (i+1) + '/' + recipients.length + ') Messaging: ' + r.name);\n"
        "    try {\n"
        "      // Step 1: Get recipient's profile URN\n"
        "      const profileResp = await fetch('https://www.linkedin.com/voyager/api/identity/profiles/' + r.public_id, {\n"
        "        headers: {'csrf-token': getCsrf(), 'x-restli-protocol-version': '2.0.0'}\n"
        "      });\n"
        "      if (!profileResp.ok) { failed++; console.log('  Profile fetch failed: ' + profileResp.status); continue; }\n"
        "      const profileData = await profileResp.json();\n"
        "      const memberUrn = profileData.entityUrn || profileData.miniProfile?.entityUrn || '';\n"
        "      // Convert miniProfile URN to messaging URN\n"
        "      const recipientUrn = memberUrn.replace('fs_miniProfile', 'fs_miniProfile');\n"
        "      if (!recipientUrn) { failed++; console.log('  No URN found for ' + r.name); continue; }\n\n"
        "      // Step 2: Send message via Voyager messaging API\n"
        "      const msgBody = {\n"
        "        conversationCreate: {\n"
        "          eventCreate: {\n"
        "            value: {\n"
        ...
        "        }\n"
        "      };\n\n"
        "      const sendResp = await fetch('https://www.linkedin.com/voyager/api/messaging/conversations?action=create', {\n"
        "        method: 'POST',\n"
        "        headers: {\n"
        "          'csrf-token': getCsrf(),\n"
        ...
        "        failed++;\n"
        "        console.log('  Failed (' + sendResp.status + '): ' + errText.substring(0, 100));\n"
        "      }\n\n"
        "      // Rate limit: 3-6 seconds between messages\n"
