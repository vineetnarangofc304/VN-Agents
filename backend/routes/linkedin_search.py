...
async def generate_message_script(data: dict):
    ...
    script = (
        "// === LinkedIn Bulk Messenger v6 ===\n"
        "// Minimal headers + credentials:include + multiple payload formats\n"
        ...
        "  console.log('%c=== Bulk Messenger v6: ' + recipients.length + ' recipients ===%c', 'color:#0a66c2;font-size:14px;font-weight:bold', '');\n\n"
        "  for (let i = 0; i < recipients.length; i++) {\n"
        "    const rec = recipients[i];\n"
        "    console.log('(' + (i+1) + '/' + recipients.length + ') ' + rec.name);\n\n"
        "    let rawUrn = rec.entity_urn || '';\n\n"
        "    // URN lookup\n"
        "    if (!rawUrn && rec.public_id) {\n"
        "      try {\n"
        ...
        "          {headers: {'csrf-token': csrf, 'x-restli-protocol-version': '2.0.0'}, credentials: 'include'}\n"
        "        );\n"
        "        if (dResp.ok) {\n"
        "          const dd = await dResp.json();\n"
        "          for (const item of (dd.included || [])) {\n"
        "            const eu = item.entityUrn || '';\n"
        "            if (eu.includes('fsd_profile') || eu.includes('miniProfile')) { rawUrn = eu; break; }\n"
        "          }\n"
        "        }\n"
        "      } catch(e) {}\n"
        "    }\n"
        "    if (!rawUrn) { skipped++; console.log('  SKIP — no URN'); await delay(500); continue; }\n\n"
        "    const memberId = rawUrn.split(':').pop();\n"
        "    console.log('  ID: ' + memberId);\n\n"
        "    let ok = false;\n\n"
        "    // === Attempt 1: XMLHttpRequest (matches browser's native request format) ===\n"
        "    if (!ok) {\n"
        "      try {\n"
        "        ok = await new Promise((resolve) => {\n"
        "          const xhr = new XMLHttpRequest();\n"
        "          xhr.open('POST', 'https://www.linkedin.com/voyager/api/messaging/conversations?action=create');\n"
        ...
        "          };\n"
        "          xhr.onerror = () => { console.log('  XHR error'); resolve(false); };\n"
        ...
        "          }));\n"
        "        });\n"
        "        if (ok) console.log('  XHR method OK');\n"
        "      } catch(e) { console.log('  XHR err: ' + e.message); }\n"
        "    }\n\n"
        "    // === Attempt 2: fetch with minimal headers + fsd_profile ===\n"
        "    if (!ok) {\n"
        "      try {\n"
        "        const r = await fetch('https://www.linkedin.com/voyager/api/messaging/conversations?action=create', {\n"
        "          method: 'POST',\n"
        "          credentials: 'include',\n"
        "          headers: {'csrf-token': csrf, 'content-type': 'application/json; charset=UTF-8', 'x-restli-protocol-version': '2.0.0'},\n"
        "          body: JSON.stringify({\n"
        "            keyVersion: 'LEGACY_INBOX',\n"
        "            conversationCreate: {\n"
        "              eventCreate: {\n"
        ...
        "          })\n"
        "        });\n"
        "        if (r.ok || r.status === 201) { ok = true; console.log('  fetch fsd OK'); }\n"
        "        else { console.log('  fetch fsd: ' + r.status); }\n"
        "      } catch(e) {}\n"
        "    }\n\n"
        "    // === Attempt 3: Navigate & use LinkedIn's own compose (DOM automation) ===\n"
        "    if (!ok && rec.public_id) {\n"
        "      try {\n"
        "        console.log('  Trying DOM method via profile...');\n"
        "        // Open messaging overlay for this person\n"
        "        const msgUrl = 'https://www.linkedin.com/messaging/compose/?connId=' + memberId;\n"
        "        const overlay = await fetch(msgUrl, {credentials: 'include', redirect: 'follow'});\n"
        "        // Try the messaging thread creation via the messaging compose endpoint\n"
        "        const composeResp = await fetch('https://www.linkedin.com/voyager/api/messaging/conversations', {\n"
        "          method: 'POST',\n"
        "          credentials: 'include',\n"
        "          headers: {'csrf-token': csrf, 'content-type': 'application/json; charset=UTF-8', 'x-restli-protocol-version': '2.0.0'},\n"
        "          body: JSON.stringify({\n"
        ...
        "          })\n"
        "        });\n"
        "        if (composeResp.ok || composeResp.status === 201) { ok = true; console.log('  compose OK'); }\n"
        "        else {\n"
        "          const et = await composeResp.text().catch(() => '');\n"
        "          console.log('  compose(' + composeResp.status + '): ' + et.substring(0, 80));\n"
        "        }\n"
        "      } catch(e) { console.log('  DOM err: ' + e.message); }\n"
        "    }\n\n"
        "    if (ok) {\n"
        "      sent++;\n"
        "      console.log('%c  SENT ✓%c', 'color:#22c55e;font-weight:bold', '');\n"
        "    } else {\n"
        "      failed++;\n"
        "      console.log('%c  FAILED — all methods returned 403%c', 'color:#ef4444;font-weight:bold', '');\n"
        "    }\n\n"
        "    await delay(3000 + Math.random() * 3000);\n"
        "  }\n\n"
        "  const summary = 'Done! Sent: ' + sent + ', Failed: ' + failed + ', Skipped: ' + skipped;\n"
        "  console.log('%c=== ' + summary + ' ===%c', 'color:#0a66c2;font-size:13px;font-weight:bold', '');\n"
        "  alert(summary);\n"
        "})();\n"
    )
